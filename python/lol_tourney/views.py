...
from lol_tourney.forms import SignUpForm, SignUpFormTwo, SummonerInfo
...
def user_info(request):
    if request.POST:
        form = SummonerInfo(request.POST, instance=Summoner.objects.get(pk=request.user.id))
        if form.is_valid():
            form.save()
            return redirect('home')
        else:
            # I sure hope it never gets here cause that form always should validate :D
            return render(request, 'user_info.html', {'form': form,})
    return render(request, 'user_info.html', {'form': SummonerInfo(instance=Summoner.objects.get(pk=request.user.id)),})
